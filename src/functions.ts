...
import { candlesAndGrandPianoPrice, candlesPrice, flowersPrice, grandPianoPrice, x2ConcuerdaDiscount, x2DifferentGroupDiscount, x2SameGroupDiscount, x3ConcuerdaDiscount, x3DifferentGroupDiscount, x3SameGroupDiscount } from "./variables";
...
export const getDiscount = (
    ...
    if (
        ...
        return ['Trio Concuerda', 'Trio Concuerda Infinity'].includes(ceremonyGroup.name) || ['Trio Concuerda', 'Trio Concuerda Infinity'].includes(cocktailGroup.name) ? x3ConcuerdaDiscount : x3SameGroupDiscount;
    ...
    if (ceremonyGroup && cocktailGroup && ceremonyGroup === cocktailGroup)
        return ['Trio Concuerda', 'Trio Concuerda Infinity'].includes(ceremonyGroup.name) ? x2ConcuerdaDiscount : x2SameGroupDiscount;

    if (cocktailGroup && feastGroup && cocktailGroup === feastGroup)
        return ['Trio Concuerda', 'Trio Concuerda Infinity'].includes(cocktailGroup.name) ? x2ConcuerdaDiscount : x2SameGroupDiscount;
